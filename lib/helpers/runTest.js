  ...
  try {
    ...
    if (packages) {
      ...
      for (const packageVersion of packageVersions) {
        const packageName = packageVersion.substring(0, packageVersion.lastIndexOf("@"));
        versions[packageName] = packageVersion.substring(packageVersion.lastIndexOf("@") + 1);
      }
    }
      ...
      for (const moduleName of modules) {
        ...
        const syntaxResults = await mapAsync(testCases, !noParallel, async ([name, content], idx) => {
          ...
          fs.writeFileSync(
            ...
            requireTest && !noRequireInHarness

...
const variantOrder = ["mjs", "js", "js-module", "mts", "ts", "ts-module"];
...
${Array.from(runtimes)
  .map(runtime => `  - [${runtime}](#${hash(runtime)})\n${variantOrder
    .map(variant => `    - [${formatVariant(variant)}](#${hash(formatResultKeyShort(`${runtime}.${variant}`))})`).join("\n")}`
  ).join("\n")
}

...
const disablingEsModuleFromESMResults = {
  defaultImport: [["`import x`", ...esmVariants]],
  dynamicImport: [["`import().default`", ...esmVariants]],
};

for (const runtime of runtimes) {
  if (runtime === "node") continue;
  const lines = {
    defaultImport: [runtime],
    dynamicImport: [runtime],
  };
  for (const variant of esmVariants) {
    ...
    for (const bucket in disablingEsModuleFromESMResults) {
      const testCase = disablingEsModuleFromESMResults[bucket][0][0];
      const line = lines[bucket];
      ...
      const pass = value.startsWith("`{") && value.includes("default") ? true : value === "`'default'`" ? false : undefined;
      line.push(pass === true ? "✅" : pass === false ? "❌" : "❓");
    }
  }
  disablingEsModuleFromESMResults.defaultImport.push(lines.defaultImport);
  disablingEsModuleFromESMResults.dynamicImport.push(lines.dynamicImport);
}
...
table. Variants that report \`'default'\` for \`import x\` and \`import().default\` relied on \`__esModule\`, while variants that
report something like \`{ [__esModule], default }\` did not. For variants that represent files that would
be recognized as ESM by Node, we would prefer to see the latter. The following tables show which runtimes
have the expected behavior for different kinds of files that should ideally be treated as ESM.

${formatTable(disablingEsModuleFromESMResults.defaultImport)}

${formatTable(disablingEsModuleFromESMResults.dynamicImport)}
